...
describe('reportParser(options).parse(file)', () => {
  ...
  it('loads one feature, one scenario, one passing step', () => {
    ...
    expect(report[0][0]).to.deep.equal({name: 'A Passing Scenario', result: 'passed'});
  })
  ...
  it('loads one feature, one scenario, one failing step', () => {
    ...
    expect(report[0][0]).to.deep.equal({name: 'A Failing Scenario', result: 'failed'});
  })
  ...
  it('loads one feature, one passing scenario, one failing scenario', () => {
    ...
    expect(report[0][0]).to.deep.equal({name: 'A Passing Scenario', result: 'passed'})
    expect(report[0][1]).to.deep.equal({name: 'A Failing Scenario', result: 'failed'})
  ...
  it('loads one feature, one scenario, some steps skipped', () => {
    ...
    expect(report[0][0]).to.deep.equal({name: 'Users can perform a search', result: 'failed'});
  })
